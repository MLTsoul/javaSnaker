...
public class SnakeEntry {
    ...
    // 蛇身所有坐标
    private LinkedList<CoordinateEntry> snakeList;
    // 下次移动是否删除最后一个
    ...
    private SnakeEntry(int mapWidth, int mapHeight) {
        ...
        for (int i = 0; i < initSnakeSize; i++) {
            initialCoo.add(new CoordinateEntry(mapWidth / 2 - i, mapHeight / 2));
        }
        initData();
    }
     ...
     * 计算下一步坐标
     */
    public CoordinateEntry nextCoordinate(){
        CoordinateEntry oldSnakeEntry = snakeList.get(0);
        CoordinateEntry snakeEntry = new CoordinateEntry(oldSnakeEntry.getCoordinate_x(), oldSnakeEntry.getCoordinate_y());
     ...
     */
    public boolean next() {
        CoordinateEntry snakeEntry = nextCoordinate();
        int x = snakeEntry.getCoordinate_x();
        int y = snakeEntry.getCoordinate_y();
        switch (motionDirection) {
            case LEFT_DIRECTION:
            ...
            case TOP_DIRECTION:
                ...
                break;
            case RIGHT_DIRECTION:
                if (x >= MapEntry.getMapWidth()) {
                    return false;
                }
                break;
            case BOTTOM_DIRECTION:
                if (y >= MapEntry.getMapHeight()) {
                    return false;
                }
                break;
        }
     ...
     * @return 是否撞到自己
     */
    public boolean isRamSelf(CoordinateEntry newCoordinate){
        for (CoordinateEntry coordinateEntry : snakeList) {
     ...
     */
    public boolean isRam(CoordinateEntry snakeEntry) {
        CoordinateEntry coordinateEntry = snakeList.get(0);
    ...
    public LinkedList<CoordinateEntry> getSnakeList() {
        return snakeList;
    }
    ...
    public void setDirectionLeft() {
        CoordinateEntry firstCoo = snakeList.get(0);
        CoordinateEntry two = snakeList.get(1);
